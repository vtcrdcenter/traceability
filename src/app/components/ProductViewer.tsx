...
export default function ProductViewer() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      environment.dispose();
      pmrem.dispose();
      scene.add(new THREE.HemisphereLight(0xfffbf3, 0x5b3430, 0.2));
      const key = new THREE.DirectionalLight(0xfff4dc, 0.42);
      ...
      const resize = () => {
        ...
        const distance = 0.9 / Math.sin(halfFov);
      ...
      const gltf = await loader.parseAsync(data, MODEL_URL.slice(0, MODEL_URL.lastIndexOf("/") + 1));
